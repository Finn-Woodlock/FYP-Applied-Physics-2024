    ...
    for (int eq : {0})
{
    ...
    for (int N : Ns)
    {
        ...
        std::vector<double> Ts = Tools::linspace(6.5,0.5,13);
        if (Ts[0] == 0) Ts[0] = 0.01;
        ...
        Alloy::Alloy newObj = Alloy::Alloy(p, 1, 1.5, true, Ls, Spins, SpinsDist, 2,brav,eq>=3,0.,getStd,getAc);

        if ((eq == 2) || (eq == 4)) {
            int id = 0;
            for (int i = 0; i+1 < (int) Spins.size();i++)
            {
                newObj.spin.row(0)(Eigen::seqN(id,(int) p.cols()*SpinsDist[i])) = Eigen::RowVectorXd::Constant((int) p.cols()*SpinsDist[i],Spins[i]);
                id += (int) p.cols()*SpinsDist[i];
            }
            newObj.spin.row(0)(Eigen::seq(id,p.cols()-1,1)) = Eigen::RowVectorXd::Constant(p.cols()-id,Spins[Spins.size()-1]);
            }
        ...
        if (!grid) {gridStr = "OFF";}
        StoreDir = "Alloy"+gridStr + std::to_string(N)+"/";
        if (eq > 0) {StoreDir = "Alloy"+gridStr + "Timed"+"/";}
       {
       ...
       }
       {
        ...
        if (newObj.E.size() > 0)
        {
            std::string headers = "T , E , C , Run";
            if (getStd) {headers = "T , E , C , EErr , CErr , Run";}
            if (getStd && getAc) {headers = "T , E , C , EErr , CErr , EAC , CAC , Run";}
            ...
            out.row(1)= Eigen::Map<Tools::DataRowVector>(newObj.E.data(),1,_Ts.size());
            out.row(2)= Eigen::Map<Tools::DataRowVector>(newObj.C.data(),1,_Ts.size());
            if (getStd) {
            out.row(3)= Eigen::Map<Tools::DataRowVector>(newObj.Ee.data(),1,_Ts.size());
            out.row(4)= Eigen::Map<Tools::DataRowVector>(newObj.Ce.data(),1,_Ts.size());
            }
            if (getStd && getAc) {
            out.row(5)= Eigen::Map<Tools::DataRowVector>(newObj.EAC.data(),1,_Ts.size());
            out.row(6)= Eigen::Map<Tools::DataRowVector>(newObj.CAC.data(),1,_Ts.size());
            }
